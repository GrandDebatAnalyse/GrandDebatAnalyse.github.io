...
stpW += ['tous','les','bonne chose', ' mauvaise chose', 'bonne', 'chose', 'seulement', 'ci dessu','votes blanc','aussi','plus','c\'est','France','etc','notamment','car', 'peut', 'être', 'devrait', 'peut être', 'doivent être', 'bonne chose', 'grand débat'
...
d = os.getcwd()
text = open(path.join(d, 'data/AllTextDemCit.txt')).read()


mask =  np.array(Image.open(path.join(d, "assembly.jpg")))
...
wc.to_file(path.join(d, "assembly_wc.png"))
