...
stpW += ['tous','les','aussi','plus','c\'est','France','etc','notamment','car', 'peut', 'être', 'devrait', 'peut être', 'doivent être', 'bonne chose', 'grand débat'
        'si','autre','autres','fait','non','donc','ainsi','surtout','alors','exemple',
         'comme','encore','dont','peut-être','cela','Oui','Non','Cest','ils','Ils','il','Il','Les','les','en', 'ça','LA','quil','Que','Si','si','faut','je','Je','chaque','dun','nest','doit', 'services', 'service', 'public', 'services public', 'service public', 'mesure', 'cette mesure' ]




d = os.getcwd()
text = open(path.join(d, 'data/AllTextOrgServ.txt')).read()


mask =  np.array(Image.open(path.join(d, "engrenage.jpg")))
...
wc.to_file(path.join(d, "OrgServPub.png"))
